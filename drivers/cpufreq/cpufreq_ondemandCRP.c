/*
 *  drivers/cpufreq/cpufreq_ondemandCRP.c
 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_ONDEMANDCRP
static
#endif
struct cpufreq_governor cpufreq_gov_ondemandCRP = {
       .name                   = "ondemandCRP",
...
static int ondemandCRP_powersave_bias_setspeed(struct cpufreq_policy *policy,
...
static void ondemandCRP_powersave_bias_init_cpu(int cpu)
{
...
static void ondemandCRP_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		ondemandCRP_powersave_bias_init_cpu(i);
	}
}
...
/* cpufreq_ondemandCRP Governor Tunables */
 ...
 * rate is 10 ms because the user needs immediate reaction from ondemandCRP
...
skip_this_cpu:
		...
		ondemandCRP_powersave_bias_init();
	} else {
		...
		for_each_online_cpu(cpu) {
			...
			if (dbs_info->cur_policy) {
				/* cpu using ondemandCRP, cancel dbs timer */
				mutex_lock(&dbs_info->timer_mutex);
				dbs_timer_exit(dbs_info);

				ondemandCRP_powersave_bias_setspeed(
					dbs_info->cur_policy,
					NULL,
					input);
...
#if 0
		/*
		 * For the purpose of ondemandCRP, waiting for disk IO is an
...
static void dbs_refresh_callback(struct work_struct *work)
{
	...
	policy = this_dbs_info->cur_policy;
	if (!policy) {
		/* CPU not using ondemandCRP governor */
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_dbs_info->rate_mult = 1;
		ondemandCRP_powersave_bias_init_cpu(cpu);
		/*
		...
		if (!ondemandCRP_powersave_bias_setspeed(
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	dbs_wq = alloc_workqueue("ondemandCRP_dbs_wq", WQ_HIGHPRI, 0);
	if (!dbs_wq) {
		printk(KERN_ERR "Failed to create ondemandCRP_dbs_wq workqueue\n");
		return -EFAULT;
	}
	...
	return cpufreq_register_governor(&cpufreq_gov_ondemandCRP);
}
...
static void __exit cpufreq_gov_dbs_exit(void)
{
	...
	cpufreq_unregister_governor(&cpufreq_gov_ondemandCRP);
	for_each_possible_cpu(i) {
		struct cpu_dbs_info_s *this_dbs_info =
			&per_cpu(od_cpu_dbs_info, i);
...
MODULE_DESCRIPTION("'cpufreq_ondemandCRP' - A dynamic cpufreq governor for "
	"Low Latency Frequency Transition capable processors");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_ONDEMANDCRP
