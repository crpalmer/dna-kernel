...
static int hold_htc_idle_wakelock = 0;
int has_htc_idle_wakelock(void)
{
	return hold_htc_idle_wakelock;
}
EXPORT_SYMBOL(has_htc_idle_wakelock);

void htc_idle_wake_lock(void)
{
	unsigned long irqflags;
	spin_lock_irqsave(&list_lock, irqflags);
	hold_htc_idle_wakelock = 1;
	spin_unlock_irqrestore(&list_lock, irqflags);
}
EXPORT_SYMBOL(htc_idle_wake_lock);

void htc_idle_wake_unlock(void)
{
	unsigned long irqflags;
	spin_lock_irqsave(&list_lock, irqflags);
	hold_htc_idle_wakelock = 0;
	spin_unlock_irqrestore(&list_lock, irqflags);

}
EXPORT_SYMBOL(htc_idle_wake_unlock);
